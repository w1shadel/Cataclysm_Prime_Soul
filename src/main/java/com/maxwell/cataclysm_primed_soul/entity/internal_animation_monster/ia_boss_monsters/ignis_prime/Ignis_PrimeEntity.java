...
public class Ignis_PrimeEntity extends IABoss_monster {
    ...
    public void tick() {
        ...
        if (this.level().isClientSide()) {
            ...
            if (this.uppercutCooldown > 0) this.uppercutCooldown--;
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(0, new IgnisStateGoal(this, STATE_ROCK_END, STATE_ROCK_END, 0, 21, 0, 20.0F) {
            ...
            public void tick() {
                super.tick();
                if (ignis.attackTicks == 10) ignis.performRockThrow();
            }
        ...
        this.goalSelector.addGoal(0, new IgnisStateGoal(this, STATE_CHARGE_SHOCKWAVE, STATE_CHARGE_SHOCKWAVE, 0, 36, 0, 20.0F));
    ...
    public void performRockThrow() {
        ...
        Cm_Falling_Block_Entity rock = new Cm_Falling_Block_Entity(this.level(), this.getX(), this.getY() + 2.5D, this.getZ(), rockState, 40);
        Vec3 dir = this.getLookAngle();
        if (target != null) {
            dir = target.position().add(0, target.getBbHeight() * 0.5, 0).subtract(this.position().add(0, 2.5, 0)).normalize();
        }
    ...
    private void handleShockwaveAction() {
        if (this.attackTicks < 20 && this.getTarget() != null) {
            float targetYaw = (float) (Mth.atan2(this.getTarget().getZ() - this.getZ(), this.getTarget().getX() - this.getX()) * (180D / Math.PI)) - 90.0F;
    ...
    private void spawnFallingBlockShockwave(int length, float arc) {
        ...
        for (int d = 1; d <= length; d++) {
            ...
            for (int i = 0; i <= points; i++) {
                if (d > 3 && this.getRandom().nextFloat() < 0.22F) continue;
                ...
                BlockState state = this.level().getBlockState(pos);
                if (!state.isAir() && state.getRenderShape() == RenderShape.MODEL) {
                    Cm_Falling_Block_Entity falling = new Cm_Falling_Block_Entity(this.level(), px, pos.getY() + 1.0D, pz, state, 16 + this.getRandom().nextInt(15));
                    falling.push(0, 0.12D + this.getRandom().nextDouble() * 0.22D + (d * 0.008D), 0);
                    this.level().addFreshEntity(falling);
                }
                AABB hitBox = new AABB(px - 1.25, this.getY() - 1.0, pz - 1.25, px + 1.25, this.getY() + 3.0, pz + 1.25);
                for (LivingEntity target : this.level().getEntitiesOfClass(LivingEntity.class, hitBox)) {
                    if (target != this && !this.isAlliedTo(target) && !alreadyHit.contains(target)) {
                        if (target.hurt(this.damageSources().mobAttack(this), (float) (this.getAttributeValue(Attributes.ATTACK_DAMAGE) * 1.5f))) {
    ...
    private boolean canDamageTarget(LivingEntity target) {
        return target != this && this.isAlive() && target.isAlive() && this.canAttack(target) && !this.isAlliedTo(target);
    }
    ...
    private void applyAttackKnockback(LivingEntity target, float knockback, double forwardPush, double verticalImpulse) {
        float yaw = this.yBodyRot * ((float) Math.PI / 180.0F);
        if (knockback > 0.0F) {
            target.knockback(knockback, Math.sin(yaw), -Math.cos(yaw));
        }
